...
void matrix_mult(double * a, double * b, double * res, int size)
{
#pragma omp parallel for
for (int i = 0; i < size; i += BLOCK) {  
    for (int j = 0; j < size; j += BLOCK) {  
        for (int k = 0; k < size; k += BLOCK) {  
            for (int kk = k; kk < std::min(k + BLOCK, size); ++kk) {  
                for (int ii = i; ii < std::min(i + BLOCK, size); ++ii) { 
#pragma omp simd
                    for (int jj = j; jj < std::min(j + BLOCK, size); ++jj) {  
